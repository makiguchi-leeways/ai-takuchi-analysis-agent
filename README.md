...
MLIT_API_KEY=...
REAL_ESTATE_LIBRARY_BASE_URL=https://www.reinfolib.mlit.go.jp/ex-api/external
ESTAT_API_KEY=...
RESAS_API_KEY=...
GOOGLE_MAPS_API_KEY=...
```
...
不動産取引価格・過去取引は国土交通省 不動産情報ライブラリの `XPT001`、用途地域・建築規制は `XKT002` をサーバー側から取得します。利用する場合は `REAL_ESTATE_LIBRARY_API_KEY` または `MLIT_API_KEY` をVercelのProductionとPreviewに設定してください。未設定時は同じレイヤー形状でプレビュー表示し、画面上は「エラー」ではなく「プレビュー」と表示します。
