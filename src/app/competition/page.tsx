  ...
  return <main className="business-screen">
    ...
    <section className="business-content">
      ...
      <div className="business-two-column">
        <section className="business-panel"><div className="panel-heading"><div><TrendingUp size={17} /><h2>自社KPI入力</h2></div><span>比較対象：上位{competitors.length}社平均</span></div><div className="own-kpi-form own-kpi-form-wide"><NumberInput label="年間販売戸数（戸・年間累計）" value={own.annualSalesUnits} onChange={(value) => updateOwn("annualSalesUnits", value)} /><NumberInput label="年間平均月末在庫戸数（戸・月末平均）" value={own.averageMonthEndInventoryUnits} onChange={(value) => updateOwn("averageMonthEndInventoryUnits", value)} /><NumberInput label="年間平均月末在庫金額（億円・月末平均）" value={own.averageInventoryValueOkuYen} onChange={(value) => updateOwn("averageInventoryValueOkuYen", value)} /><NumberInput label="年間売上原価（億円・年間累計）" value={own.costOfSalesOkuYen} onChange={(value) => updateOwn("costOfSalesOkuYen", value)} /><NumberInput label="年間売上総利益（億円・年間累計）" value={own.grossProfitOkuYen} onChange={(value) => updateOwn("grossProfitOkuYen", value)} /></div><p className="muted-note">期間の定義：販売戸数・売上原価・売上総利益は直近12か月または決算年度の年間累計。在庫戸数・在庫金額は12か月分の月末残高の平均です。回転数 = 年間販売戸数 ÷ 年間平均月末在庫戸数。販売期間 = 12 ÷ 回転数。</p></section>
      ...
      <CompetitionComparison own={ownKpi} competitor={selectedCompetitor} />
      <section className="business-panel formula-panel"><div className="panel-heading"><div><Calculator size={17} /><h2>在庫回転KPIの計算根拠</h2></div><span>財務諸表・決算説明資料から推定</span></div><div className="formula-grid"><Formula label="年間平均月末在庫戸数" value="12か月の月末在庫戸数合計 ÷ 12" note="期末在庫だけでなく、年間の滞留を平均化" /><Formula label="戸数ベース在庫回転数" value="年間販売戸数 ÷ 年間平均月末在庫戸数" note="仕入れた戸数が販売に回る速さ" /><Formula label="販売期間" value="12か月 ÷ 戸数ベース在庫回転数" note="平均的に在庫が販売されるまでの月数" /><Formula label="金額ベース在庫回転率" value="年間売上原価 ÷ 年間平均月末在庫金額" note="年間累計の売上原価と月末在庫の平均を比較" /></div><p className="muted-note">販売戸数が開示されない会社は、売上高 ÷ 平均販売価格で戸数を推定します。推定値には「推定」を付け、実績値と混同しない表示にします。</p></section>
...
function formatUnits(value: number | null) {
  return value === null ? "データなし" : `${value.toFixed(1)}戸`;
}

function formatOkuYen(value: number | null) {
  return value === null ? "データなし" : `${value.toFixed(1)}億円`;
}
...
function SelectedCompanyDetail({ competitor }: { competitor: Competitor }) {
  ...
  return <div className="selected-company-detail"><strong>{competitor.name}</strong><span>重点仕入地域</span><b>{competitor.focus}</b><div className="company-kpi-grid"><Metric label="年間販売戸数（年間累計）" value={`${kpi.annualSalesUnits?.toFixed(0) ?? "データなし"}戸${kpi.annualSalesUnitsEstimated ? "（推定）" : ""}`} /><Metric label="年間平均月末在庫（12か月平均）" value={`${kpi.averageMonthEndInventoryUnits.toFixed(1)}戸`} /><Metric label="販売期間" value={formatMonths(kpi.salesPeriodMonths)} /><Metric label="在庫回転日数" value={kpi.inventoryDays ? `${kpi.inventoryDays.toFixed(0)}日` : "データなし"} /><Metric label="年間売上原価（年間累計）" value={`${kpi.costOfSalesOkuYen.toFixed(1)}億円`} /><Metric label="金額回転率" value={formatTimes(kpi.valueTurnover)} /></div><details className="month-end-details"><summary>12か月の月末在庫戸数を見る</summary><p>{competitor.financials.monthEndInventoryUnits.map((value, index) => `${index + 1}月 ${value}戸`).join(" / ")}</p></details><small>原典：EDINET・企業IR。実データ接続後は有価証券報告書の年度・項目・原典URLを保持します。</small></div>;
}

function CompetitionComparison({ own, competitor }: { own: CompetitionKpi; competitor: Competitor }) {
  const rows: ComparisonMetric[] = [
    { label: "戸数ベース在庫回転数", own: own.unitTurnover, competitor: competitor.kpi.unitTurnover, format: formatTimes, higherIsBetter: true },
    { label: "販売期間", own: own.salesPeriodMonths, competitor: competitor.kpi.salesPeriodMonths, format: formatMonths, higherIsBetter: false },
    { label: "金額ベース在庫回転率", own: own.valueTurnover, competitor: competitor.kpi.valueTurnover, format: formatTimes, higherIsBetter: true },
    { label: "年間販売戸数", own: own.annualSalesUnits, competitor: competitor.kpi.annualSalesUnits, format: formatUnits, higherIsBetter: true },
    { label: "年間平均月末在庫戸数", own: own.averageMonthEndInventoryUnits, competitor: competitor.kpi.averageMonthEndInventoryUnits, format: formatUnits, higherIsBetter: false },
    { label: "平均月末在庫金額", own: own.averageInventoryValueOkuYen, competitor: competitor.kpi.averageInventoryValueOkuYen, format: formatOkuYen, higherIsBetter: false },
    { label: "売上総利益率", own: own.grossMarginRate, competitor: competitor.kpi.grossMarginRate, format: formatPercent, higherIsBetter: true }
  ];

  return <section className="business-panel company-comparison-panel"><div className="panel-heading"><div><BarChart3 size={17} /><h2>自社KPIと競合企業を比較</h2></div><span>選択中：{competitor.name}</span></div><p className="muted-note">自社入力値と選択した競合企業の同一年度KPIを比較しています。競合優位・自社優位は各指標の目的に応じて判定します。</p><div className="company-comparison-table"><div className="company-comparison-head"><span>指標</span><strong>自社</strong><strong>{competitor.name}</strong><strong>判定</strong></div>{rows.map((row) => <ComparisonRow key={row.label} row={row} />)}</div></section>;
}
...
function ComparisonRow({ row }: { row: ComparisonMetric }) {
  const delta = row.own !== null && row.competitor !== null ? row.own - row.competitor : null;
  const meaningfulDelta = delta !== null && Math.abs(delta) >= 0.05;
  const ownWins = delta !== null && meaningfulDelta && (row.higherIsBetter ? delta > 0 : delta < 0);
  const competitorWins = delta !== null && meaningfulDelta && !ownWins;
  const result = delta === null ? "比較不可" : meaningfulDelta ? ownWins ? "自社優位" : "競合優位" : "同水準";
  const resultTone = ownWins ? "own" : competitorWins ? "competitor" : "neutral";
  return <div className="company-comparison-row"><span><strong>{row.label}</strong><small>差分（自社 − 競合）{delta === null ? "：データなし" : `：${formatSigned(delta, row)}`}</small></span><b>{row.format(row.own)}</b><b>{row.format(row.competitor)}</b><em className={`comparison-result-${resultTone}`}>{result}</em></div>;
}

function formatSigned(value: number, row: ComparisonMetric) {
  const sign = value > 0 ? "+" : "";
  if (row.format === formatTimes) return `${sign}${value.toFixed(1)}回`;
  if (row.format === formatMonths) return `${sign}${value.toFixed(1)}か月`;
  if (row.format === formatUnits) return `${sign}${value.toFixed(1)}戸`;
  if (row.format === formatOkuYen) return `${sign}${value.toFixed(1)}億円`;
